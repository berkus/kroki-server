...
public class Gnuplot implements DiagramService {
  ...
  public Gnuplot(Vertx vertx, JsonObject config, Commander commander) {
    this.vertx = vertx;
    this.binPath = config.getString("KROKI_GNUPLOT_BIN_PATH", "gnuplot");
  ...
  public void convert(String sourceDecoded, String serviceName, FileFormat fileFormat, JsonObject options, Handler<AsyncResult<Buffer>> handler) {
    vertx.executeBlocking(future -> {
      try {
        byte[] result = gnuplot(sourceDecoded.getBytes());
  ...
  private byte[] gnuplot(byte[] source) throws IOException, InterruptedException, IllegalStateException {
